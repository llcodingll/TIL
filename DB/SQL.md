...
|----------------------------------------------------|--------------------------------|
| `CONCAT(a, b)`                                     | 문자열 연결                         |
| `SUBSTRING(str, start, len)` //(문자열, 시작 위치, 글자 개수) | 문자열 일부 추출 // 인덱스 1부터 시작! 0 아님! |
| `LOWER()/UPPER()`                                  | 소문자/대문자 변환                     |
| `LENGTH()`                                         | 바이트 길이 반환 (한글은 2바이트로 계산됨)      |
...
| `NOW()`                         | 현재 날짜/시간 |
| `CURDATE()`                     | 현재 날짜    |
| `DATE_FORMAT(date, '%Y-%m-%d')` | 형식 지정 출력 |

### 집계 함수
| 함수        | 설명      | 예시                          |
|-----------|---------|-----------------------------|
| `COUNT()` | 행의 수를 셈 | `COUNT(*)`, `COUNT(column)` |
| `SUM()`   | 합계 계산   | `SUM(price)`                |
| `AVG()`   | 평균 계산   | `AVG(score)`                |
| `MAX()`   | 최댓값     | `MAX(age)`                  |
| `MIN()`   | 최솟값     | `MIN(age)`                  |
